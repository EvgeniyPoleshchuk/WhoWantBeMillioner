...
fun StatisticScreen(navController: NavController, application: Application) {
    ...
    var userList by remember { mutableStateOf(emptyList<ResulInfo>()) }

    LaunchedEffect(Unit) {
        val users = questionsRepository.getAllUsersFromCache()
        userList = users
    }
        ...
        LazyColumn(
            ...
            items(userList.size) {
                Row(
                    ...
                    Text(
                        text = userList[it].name,
                        fontSize = 18.sp,
                        fontWeight = FontWeight.Bold,
                        color = Color(0xFFFFB340)
                    )
                    Text(
                        text = userList[it].cash,
                        fontSize = 18.sp,
                        fontWeight = FontWeight.Bold,
                        color = Color.White
                    )
